...
# Each pattern is defined relative to the candidate move (treated as one
# endpoint of a potential bridge).
# (bx_off, by_off)  -> other endpoint
# (c1_off) and (c2_off) -> the two cut points between the endpoints
BRIDGE_PATTERNS = [
    ((-1, -1), (-1, 0), (0, -1)),    # roughly top-left direction
...
# Moves that are close to opponent's last move are prioritised
...
def bridge_score(board: Bitboard, colour: Colour, candidate_move: tuple[int, int]) -> int:
    """
    Small bonus if playing candidate_move would create a clean bridge
    with one of our existing stones.

    Conditions for a bonus in one pattern:
      - Other endpoint is our colour.
      - Both intermediate 'cut' cells are empty.

    The bonus is small so it only nudges move ordering and
    does not fight against what MCTS + RAVE learn from rollouts.
    """
    x, y = candidate_move
    size = board.size
    bonus = 0

    for (bx_off, by_off), (c1_off, c1y_off), (c2_off, c2y_off) in BRIDGE_PATTERNS:
        bx, by = x + bx_off, y + by_off
        c1x, c1y = x + c1_off, y + c1y_off
        c2x, c2y = x + c2_off, y + c2y_off

        # All cells must be on board
        if not (0 <= bx < size and 0 <= by < size and
                0 <= c1x < size and 0 <= c1y < size and
                0 <= c2x < size and 0 <= c2y < size):
            continue

        # Other endpoint must be our stone; cut points must both be empty
        if (board.colour_at(bx, by) == colour and
            board.colour_at(c1x, c1y) is None and
                board.colour_at(c2x, c2y) is None):
            bonus += 2  # small
            if bonus >= 4:
                break  # cap the effect so it stays gentle
...
def score_move(board: Bitboard, colour: Colour, candidate_move: tuple[int, int], opp_move: tuple[int, int] | None):
    ...
    # NEW: add a small, safe bridge bonus as a tie-breaker
    score += bridge_score(board, colour, candidate_move)
...
def find_forced_win(board: Bitboard, colour: Colour):
    for move in board.legal_moves():
        ...
        won = (colour == Colour.RED and board.red_won()) or (
            colour == Colour.BLUE and board.blue_won())
        ...
        try:
            while legal_moves:
                ...
                if last_move is not None and use_local:
                    lx, ly = last_move
                    local = [m for m in NEIGHBOUR_TABLE[last_move]
                             if m in legal_moves_set]
                    if local:
...
class BridgeMCTS(AgentBase):
    ...
    def make_move(self, turn: int, board: Board, opp_move: Move | None) -> Move:
                ...
                if opp_move.x != -1 and opp_move.y != -1:
                    ...
                    for child in self.root.children:
                        if child.move == target:
                            ...
                            break
                    if found == False:
                        self.root = Node(self.colour, None,
                                         None, bitboard, self.colour)
                        self.root_depth = self.root.depth
                else:
                    # Pie rule used, easiest approach is to rebuild the Node, this can be improved
                    self.root = Node(self.colour, None, None,
                                     bitboard, self.colour)
                    self.root_depth = self.root.depth
            else:
        ...
        best_child, response, iterations = self.root.search(
            move_limit, self.root_depth)
        end_time = time.time()
