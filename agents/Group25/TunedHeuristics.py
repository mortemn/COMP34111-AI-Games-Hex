...
class Node:
    ...
    def simulate(self):
        ...
        while True:
            ...
            # adjacency to friendly stones
            if move is None and random.random() < FRIEND_PROB:
                friend_adj = []
                for (x, y) in moves:
                    for dx, dy in BRIDGE_NEIGHBOURS:
                        nx, ny = x + dx, y + dy
                        if 0 <= nx < size and 0 <= ny < size:
                            if new_board.colour_at(nx, ny) == colour:
                                friend_adj.append((x, y))
                                break
                if friend_adj:
                    move = random.choice(friend_adj)

            # remaining: pure random
            if move is None:
